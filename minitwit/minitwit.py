...
def user_profile(profile_username):
    ...
    return render_template('user_profile.html', messages=[], followed=False,
            profile_username=profile_username)


@app.route('/<profile_username>/follow')
def follow_user(profile_username):
    """Adds the current user as follower of the profile user."""
    if not g.current_username:
        abort(401)
    if profile_username not in db['all_users']:
        abort(404)
...
def unfollow_user(profile_username):
    """Removes the current user as follower of the profile user."""
    if not g.current_username:
        abort(401)
    if profile_username not in db['all_users']:
        abort(404)
